...
sub just_pm_to_blib {
    ...
    return 'no lib/'
        unless $has_lib;
    # strictly ExtUtils::MakeMaker uses the pm_to_blib target to install .pm,
    # .pod and .pl files. We're just going to do it for .pm and .pod files, to
    # avoid problems on case munging file systems. Specifically, _pm.PL which
    # ExtUtils::MakeMaker should run munges to _PM.PL, and looks a lot like a
    # regular foo.pl (ie FOO.PL)
    my @pm;
    require File::Find;
    unless (eval {
        File::Find::find({
